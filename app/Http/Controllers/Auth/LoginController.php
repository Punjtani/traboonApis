...
class LoginController extends Controller
{
    ...
    */
    ...
    public function socialCallBack($provider)
    {
        ...
        if(empty($user)){
            return redirect()->route('auth.login')->with('error',__('Can not authorize'));
        }
        ...
        if(empty($existUser)){
            ...
            if(!empty($userByEmail)){
                if(in_array($userByEmail->status,['blocked'])){
                    return redirect()->route('auth.login')->with('error',__('Your account has been blocked'));
                }
            ...
            if(!empty($realUser->id)){
                if($realUser->getMeta('social_'.$provider.'_id') == $user->getId()){
                    ...
                    return redirect()->route('auth.login')->with('error',__('Email :email exists. Please select another one',['email'=>$user->getEmail()]));
                }
            }
            ...
            if($existUser->deleted == 1){
                return redirect()->route('auth.login')->with('error',__('User blocked'));
            }
