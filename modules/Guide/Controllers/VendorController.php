...
class VendorController extends FrontendController
{
    ...
    public function index(Request $request)
    {
        ...
        $user_id = Auth::id();
        $list_guide = $this->guideClass::where("create_user", $user_id)->orderBy('id', 'desc');
        $data = [
            'rows' => $list_guide->paginate(5),
            'breadcrumbs'        => [
                [
                    'name' => __('Manage Guides'),
            ...
            'page_title'         => __("Manage Guides"),
        ];
        return view('Guide::frontend.vendorGuide.index', $data);
    }
    ...
    public function create(Request $request)
    {
        ...
        $data = [
            ...
            'guide_location' => $this->locationClass::get()->toTree(),
            ...
            'page_title'         => __("Create Guides"),
        ];
        return view('Guide::frontend.vendorGuide.detail', $data);
    }
    ...
    public function store( Request $request, $id ){
        ...
        if($id>0){
            ...
            if (empty($row)) {
                return redirect(route('guide.vendor.index'));
            }
            ...
            $row = new $this->guideClass();
            $row->status = "publish";
            if(setting_item("guide_vendor_create_service_must_approved_by_admin", 0)){
                $row->status = "pending";
            }
        }
        $dataKeys = [
        ...
        if ($res) {
            ...
            if($id > 0 ){
                ...
                return redirect(route('guide.vendor.edit',['id'=>$row->id]))->with('success', __('Guide created') );
            }
        }
    }
    ...
    public function edit(Request $request, $id)
    {
        $this->checkPermission('guide_update');
        $user_id = Auth::id();
        $row = $this->guideClass::where("create_user", $user_id);
        $row = $row->find($id);
        if (empty($row)) {
            return redirect(route('guide.vendor.index'))->with('warning', __('Space not found!'));
        }
    ...
    public function delete($id)
    {
        ...
        $user_id = Auth::id();
        $query = $this->guideClass::where("create_user", $user_id)->where("id", $id)->first();
        if(!empty($query)){
            $query->delete();
        }
        return redirect(route('guide.vendor.index'))->with('success', __('Delete guide success!'));
    }

    public function bulkEditGuide($id , Request $request){
        $this->checkPermission('guide_update');
    ...
    {
        $data = [
            'bookings' => $this->bookingClass::getBookingHistory($request->input('status'), false , Auth::id() , 'guide'),
            'statues'  => config('booking.statuses'),
            'breadcrumbs'        => [
                [
                    'name' => __('Manage Guide'),
                    'url'  => route('guide.vendor.index')
                ],
                [
        ...
        return view('Guide::frontend.vendorGuide.bookingReport', $data);
    }

    public function bookingReportBulkEdit($booking_id , Request $request){
        $status = $request->input('status');
        if (!empty(setting_item("guide_allow_vendor_can_change_their_booking_status")) and !empty($status) and !empty($booking_id)) {
