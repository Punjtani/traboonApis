    ...
    <div class="tabbable">
        <ul class="nav nav-tabs ht-nav-tabs">
            ...
            <li class="@if(empty($status_type)) active @endif">
                <a href="{{ route("guide.vendor.booking_report") }}">{{__("All Booking")}}</a>
            </li>
            @if(!empty($statues))
                @foreach($statues as $status)
                    <li class="@if(!empty($status_type) && $status_type == $status) active @endif">
                        <a href="{{ route("guide.vendor.booking_report",['status' => $status]) }}">{{booking_status_to_text($status)}}</a>
                    </li>
                @endforeach
            @endif
        </ul>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-booking-history">
                        ...
                        @foreach($bookings as $booking)
                            @include('Guide::frontend.vendorGuide.bookingReport.loop')
                        @endforeach
                        </tbody>
                    </table>
                </div>
