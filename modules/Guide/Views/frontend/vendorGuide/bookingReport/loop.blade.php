<tr>
    ...
    <td class="a-hidden">
        ...
        @php $rooms = \Modules\Guide\Models\GuideRoomBooking::getByBookingId($booking->id) @endphp
        @if(!empty($rooms))
    ...
    <td width="2%">
        ...
        @endif
        @if(!empty(setting_item("guide_allow_vendor_can_change_their_booking_status")))
            ...
            <div class="dropdown-menu">
                @if(!empty($statues))
                    @foreach($statues as $status)
                        <a href="{{ route("guide.vendor.booking_report.bulk_edit" , ['id'=>$booking->id , 'status'=>$status]) }}">
