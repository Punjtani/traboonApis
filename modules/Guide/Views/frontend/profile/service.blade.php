<?php
if(!$user->hasPermissionTo('guide_create')) return;
$services = \Modules\Guide\Models\Guide::getVendorServicesQuery($user->id)->orderBy('id','desc')->paginate(6);
?>
@if($services->total())
    <div class="bravo-profile-list-services">
        @include('Guide::frontend.blocks.list-guide.index', ['rows'=>$services,'style_list'=>'normal','desc'=>' ','title'=>!empty($view_all) ? __('Space by :name',['name'=>$user->first_name]) :'','col'=>4])
