@extends('layouts.app')
@section('head')
    <link href="{{ asset('module/virtual/css/virtual.css?_ver='.config('app.version')) }}" rel="stylesheet">
    <link rel="stylesheet" type="text/css" href="{{ asset("libs/ion_rangeslider/css/ion.rangeSlider.min.css") }}"/>
    <link rel="stylesheet" type="text/css" href="{{ asset("libs/fotorama/fotorama.css") }}"/>
@endsection
@section('content')
    <div class="bravo_detail_virtual">
        @include('Virtual::frontend.layouts.details.virtual-banner')
        <div class="bravo_content">
            <div class="container">
                <div class="row py-5">
                    <div class="col-md-12 col-lg-8">
                        <h3>{{$translation->name}}</h3>
                        @include('Virtual::frontend.layouts.details.virtual-overview')
                    </div>
                ...
                @if(!empty($types))
                    <div class="g-virtual-module py-5 border-top border-bottom">
                        <div class="row">
                            <div class="col-12 col-lg-5">
                                <h3>{{__("Explore the place")}}</h3>
                            </div>
                            <div class="col-12 col-lg-7">
                                <ul class="virtual-module-nav nav nav-pills justify-content-lg-end">
                                    @php $i = 0 @endphp
                                    @foreach($types as $type=>$moduleClass)
                                        ...
                                        ->join('bravo_virtuals', function ($join) use ($row,$moduleInst) {
                                            $join->on('bravo_virtuals.id', '=', $moduleInst::getTableName().'.virtual_id')
                                                ;
                                        })
                                        ->where($moduleInst::getTableName().'.status','publish')->with('virtual')->take(8)->get();
                                        @endphp
            ...
            @include('Virtual::frontend.layouts.details.virtual-map')
            <div class="container">
                @include('Virtual::frontend.layouts.details.virtual-trip-idea')
            </div>
        </div>
    </div>
@endsection
