...
@section('content')
    <form action="{{route('virtual.admin.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
        @csrf
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    <h1 class="title-bar">{{$row->id ? 'Edit: '.$row->name : __("Add new virtual")}}</h1>
                    @if($row->slug)
                        <p class="item-url-demo">{{__("Permalink")}}: {{ url( (request()->query('lang') ? request()->query('lang').'/' : '').config('virtual.virtual_route_prefix'))  }}/<a href="#" class="open-edit-input" data-name="slug">{{$row->slug}}</a></p>
                    @endif
                </div>
                <div class="">
                    @if($row->slug)
                    ...
                    <div class="col-md-9">
                        <div class="panel">
                            <div class="panel-body">
                                <h3 class="panel-body-title">{{__("Virtual Content")}}</h3>
                                @include('Virtual::admin/form')
                                @if(is_default_lang())
                                    ...
                                    <div class="form-group form-index-hide">
                                        ...
                                        <div class="control-map-group">
                                            ...
                                            <div class="g-control">
                                                <div class="form-group">
                                                    <label>{{__("Location Lat")}}:</label>
                                                    <input type="text" name="lat" class="form-control" value="{{$row->lat}}">
                                                </div>
                                                <div class="form-group">
                                                    <label>{{__("Location Lng")}}:</label>
                                                    <input type="text" name="long" class="form-control" value="{{$row->long}}">
                                                </div>
                                                <div class="form-group">
            ...
            new BravoMapEngine('map_content', {
                ...
                ready: function (engineMap) {
                    ...
                    engineMap.on('click', function (dataLatLng) {
                        ...
                        $("input[name=lat]").attr("value", dataLatLng[0]);
                        $("input[name=long]").attr("value", dataLatLng[1]);
                    });
