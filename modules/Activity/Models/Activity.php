...
{
    use SoftDeletes;
    protected $table                              = 'bravo_activitys';
    ...
    {
        return $this->hasMany($this->activityTermClass, "target_id");
    }

    public function termsByAttributeInListingPage()
    {
        $attribute = setting_item("activity_attribute_show_in_listing_page", 0);
        return $this->hasManyThrough($this->termClass, $this->activityTermClass, 'target_id', 'id', 'id', 'term_id')->where('bravo_terms.attr_id', $attribute)->with(['translations']);
    }

    public function getAttributeInListingPage()
    {
        $attribute_id = setting_item("activity_attribute_show_in_listing_page", 0);
    ...
    public function getImageLink() {
        ...
        $link = FileHelper::url($this->image_id, 'thumb');
...
//        }
    ...
    {
        return url(route('activity.admin.edit', ['id' => $this->id]));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $success = false;
        $bulkId = Auth::id().date('Y-m-d-').mt_rand(10000000,99999999);
        foreach($request->input('data') as $service)
        {
            $discount = 0;
            $total = intval($service['service_price']);
            $total_before_fees = $total;
            $list_fees = setting_item('activity'.$service['service_type'].'_booking_buyer_fees');
            if (!empty($list_fees)) {
            ...
            $booking->object_id = $service['service_id'];//$request->input('service_id');
            $booking->object_model = $service['service_type'];//$request->input('service_type');
            $booking->vendor_id = $this->create_user;
            $booking->customer_id = Auth::id();
            $booking->total = $total;
            $booking->bulk_id = $bulkId;
            ...
            if ($check) {
                ...
                /*
                $activityBooking = new $this->activityBookingClass();
                $activityBooking->fillByAttr([
                    'room_id',
                    'parent_id',
                    'start_date',
                    'end_date',
        ...
        {
            $this->sendSuccess([
                    'url' => $booking->getCheckoutUrlMany()
                ]);
        }
    ...
    public function isAvailableInRanges($start_date, $end_date)
    {
        ...
        if ($this->default_state) {
            $notAvailableDates = $this->activityDateClass::query()->where([
                [
                    'start_date',
                    '>=',
                    $start_date
            ...
            $availableDates = $this->activityDateClass::query()->where([
                [
                    'start_date',
                    '>=',
                    $start_date
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if (empty($number))
            return false;
        if ($number > 1) {
            return __(":number Activitys", ['number' => $number]);
        }
    ...
    public function getActivityAvailability($filters = [])
    {
        $activity = $this;
        $res = [];
        if ($activity->isAvailableAt($filters)) {
                $translation = $activity->translateOrOrigin(app()->getLocale());
                $res[] = [
                    'id'              => $activity->id,
                    'title'           => $translation->title,
                    'price'           => $activity->tmp_price ?? 0,                    
                    'days'            => $activity->tmp_days ?? 0,
                    'price_html'      => format_money($activity->tmp_price) . '<span class="unit">/' . ($activity->tmp_days ? __(':count days', ['count' => $activity->tmp_days]) : __(":count day", ['count' => $activity->tmp_days])) . '</span>'
                ];
            }
        return $res;
    }
    public function isAvailableAt($filters = []){
        ...
        $activityDates =  $this->getDatesInRange($filters['start_date'],$filters['end_date']);
        $allDates = [];
        $tmp_price = 0;
        $tmp_day = 0;
        ...
        $activityBookings = $this->getBookingsInRange($filters['start_date'],$filters['end_date']);
        if(!empty($activityBookings)){
            foreach ($activityBookings as $activityBooking){
                for($i = strtotime($activityBooking->start_date); $i <= strtotime($activityBooking->end_date); $i+= DAY_IN_SECONDS)
                {
                    if(!array_key_exists(date('Y-m-d',$i),$allDates)) continue;
                    $allDates[date('Y-m-d',$i)]['number'] -= $activityBooking->number;
