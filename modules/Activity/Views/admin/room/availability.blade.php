...
@section ('content')
    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{__("Activity Availability Calendar")}}</h1>
        </div>
        ...
        @else
            <div class="alert alert-warning">{{__("No activitys found")}}</div>
        @endif
    ...
    <script>
        ...
        $('#items_tab').on('show.bs.tab',function (e) {
			...
			calendar = new FullCalendar.Calendar(calendarEl, {
				...
				events:{
                    	url:"{{route('activity.admin.room.availability.loadDates',['activity_id'=>$activity->id])}}",
						extraParams:{
							id:lastId,
                        }
                },
        ...
        formModal = new Vue({
            ...
            methods:{
                ...
                saveForm:function () {
                    ...
                    console.log('{{route('activity.admin.room.availability.store',['activity_id'=>$activity->id])}}');
                    var me = this;
                    ...
                    $.ajax({
                        url:'{{route('activity.admin.room.availability.store',['activity_id'=>$activity->id])}}',
                        data:this.form,
                        dataType:'json',
                        method:'post',
