(function ($) {
    var guideForm = new Vue({
        ...
        computed:{
            total_price:function(){
                var me = this;
                if (me.start_date !== "" && me.total_days > 0) {
                    
                    var total_price = 0;
                    _.forEach(this.rooms,function (item) {
                        if(item.days){
                            total_price += item.price* parseInt(item.days);
                        }
                    });
        ...
        methods:{
            ...
            checkAvailability:function () {
                ...
                $.ajax({
                    url:bookingCore.url+'/guide/checkAvailability',
                    data:{
                        guide_id:this.id,
    ...
    $(".start_room_sticky").each(function () {
        var $this_list_room = $(this).closest(".guide_rooms_form");
        $(window).scroll(function() {
            ...
            if (stickyTop < windowTop && windowTop < stickyBottom) {
                $(document).find(".guide_room_book_status").addClass("sticky").css("width",$this_list_room.width());
                $(document).find(".end_room_sticky").css("min-height",$(document).find(".guide_room_book_status").height() + 32 + 20);

                setTimeout(function () {
                    $(document).find(".guide_room_book_status").addClass("active");
                },100);
            } else {
                $(document).find(".guide_room_book_status").removeClass("sticky").css("width","auto");
                $(document).find(".end_room_sticky").css("min-height","auto");
                $(document).find(".guide_room_book_status").removeClass("active");
            }
        });
    });
